...
以下决定影响体验，不应由架构偏好替代。评审前可按括号内默认值推进原型：

1. 同一 Agent 是否可以在自己的消息后连续补充？（默认允许，最多连续 2 次）
2. “更正消息主话题”是否属于 P0 演示？（默认只观察，不提供编辑 UI）
3. 未被选中的高分 Agent 在分析区显示为“想回应但未选中”是否符合产品语言？
   （默认显示）
4. 默认 5 条 Agent 消息、6 轮、30,000 Token、90 秒是否符合可接受成本？
   （默认采用，UI 可见但普通用户不可在运行中扩大）
5. 首选模型供应商、具体轻量模型及其数据保留政策。（架构不预选商业供应商）
6. 首版是否需要删除单个会话，还是清空本地数据库即可？（默认提供删除会话）
